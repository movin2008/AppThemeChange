...
public final class DLSoLibManager {

    private static final String TAG = DLSoLibManager.class.getSimpleName();
     ...
     */
    private static DLSoLibManager sInstance = new DLSoLibManager();
    /**
     * app's lib dir
     */
    private static String sNativeLibDir = "";

    private DLSoLibManager() {
    }

    /**
     * @return
     */
    public static DLSoLibManager getSoLoader() {
        return sInstance;
    }
        ...
        try {
            ...
            while (entries.hasMoreElements()) {
                ...
                if (zipEntryName.endsWith(".so") && zipEntryName.contains(cpuArchitect)) {
                    ...
                    if (lastModify == getSoLastModifiedTime(context, zipEntryName)) {
                        // exist and no change
    ...
    private void setSoLastModifiedTime(Context cxt, String soName, long time) {
        SharedPreferences prefs = cxt.getSharedPreferences(PREFERENCE_NAME, Context.MODE_PRIVATE | Context.MODE_MULTI_PROCESS);
